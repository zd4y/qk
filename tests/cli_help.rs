...
FLAGS:
    ...
    -T, --list-templates    
        ...
        --overwrite         
            Overwrite the project if it already exists instead of just opening
            it
    -V, --version           
...
OPTIONS:
    -c, --config <config>    
            Specify alternative configuration file [env: QK_CONFIG_PATH=]

    -e, --editor <editor>    
            Editor to open in this project's directory. Set this to an empty
            string to skip opening an editor. If not specified, it will be
            searched in these places in order:
...
ARGS:
    ...
    <extra-args>...    
            Extra arguments for the commands in the template. If an argument
            starts with a leading hyphen (-) you must use '--' so that the
            program knows that only positional arguments follow.
...
OPTIONS:
    -c, --config <config>    Specify alternative configuration file [env:
                             QK_CONFIG_PATH=]
...
ARGS:
    ...
    <extra-args>...    Extra arguments for the commands in the template
";
